...
def is_lookalike(domain):
    for brand in KNOWN_BRANDS:
        # Einfache Checks: 1. Punycode, 2. Levenshtein-Distanz <=2, 3. Enthält Brandname
...
def analyze_attachments(msg):
    dangerous_ext = {'.exe', '.js', '.scr', '.bat', '.cmd', '.vbs', '.jar', '.zip', '.rar', '.ace', '.msi', '.ps1'}
    attachments = []
    dangerous = []
    encrypted = []
    for part in msg.walk():
        if part.get_content_disposition() == 'attachment':
            ...
            # Prüfe auf verschlüsselte/Passwort-Archive (nur grob, z.B. "encrypted" im Namen)
...
def count_addresses(header_val):
    if not header_val:
        return 0
...
def analyze_headers(msg):
    ...
    if spf:
        ...
        results["spf"] = "unbekannt"
    # DKIM
    dkim = msg.get("DKIM-Signature")
    if dkim:
        results["dkim"] = "vorhanden"
    else:
        results["dkim"] = "fehlt"
    # DMARC
...
def analyze_external_images(msg):
    external_images = []
    tracking_pixels = []
    for part in msg.walk():
